...
import { InitParticipante, Particpante, ACCION_PARTICIPANTE } from '../model/Particiante';
...
const RegisterPage = () => {
  ...
  // useEffect(() => {
  //   // localStorage.setItem('participantes', JSON.stringify(state.participantes))
  // }, [state, init])
  ...
  const submitForm = (ev: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isTerm) {
      ...
      let listP = [...actionAdd.payload.participantes, actionAdd.payload.selectedParticipante!];
